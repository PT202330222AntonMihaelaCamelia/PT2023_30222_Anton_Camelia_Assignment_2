...
public class SimulationManager implements Runnable {
    ...
    public void run() {
        ...
        try {print = new FileWriter("text.txt");} catch (IOException e) {throw new RuntimeException(e);}
        try {print1=new FileWriter("output.txt");} catch (IOException e) {throw new RuntimeException(e);}
        int stopGeneratedTask=-1, currentTime=0;
        int peakHour=0,sericeTime=0,serviceNumber=0,st=0;
        int waiting=0;
        ...
        while(currentTime <timeLimit && currentTime!=stopGeneratedTask) {
            ...
            if(generatedTasks.size()==1) stopGeneratedTask=generatedTasks.get(0).getArrivalTime()+generatedTasks.get(0).getServiceTime()+2;
            try {print.write("Waiting clients:");} catch (IOException e) {throw new RuntimeException(e);}
            for(Task t: generatedTasks) {try {print.write("("+t.getId()+","+t.getArrivalTime()+","+t.getServiceTime()+")"+",");}  catch (IOException e) {throw new RuntimeException(e);}}
            try {print.write("\n");} catch (IOException e) {throw new RuntimeException(e);}
            int i=0,count=0;
            while(i < scheduler.getServers().size()) {
                try { print.write("Queue " + i + ":");} catch (IOException e) {throw new RuntimeException(e);}
                Task[] tasks = scheduler.getServers().get(i).getTasks();
                if(scheduler.getServers().get(i).getTasks2().size()==0) try {print.write("closed ");} catch (IOException e) {throw new RuntimeException(e);}
                count=0;
                for (Task task : tasks) {
                    if(task!=null) {
                        count++;
